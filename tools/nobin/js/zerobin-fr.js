    ...
    if (comments[0].meta.opendiscussion) {
        ...
        for (var i = 1; i < comments.length; i++) {
            ...
            if (comment.meta.vizhash) {
                divComment.find('span.nickname').before('<img src="' + comment.meta.vizhash + '" class="vizhash" title="Avatar anonyme (Vizhash de l’adresse IP)" />');
            }
...
function send_comment(parentid) {
    ...
    $.post(scriptLocation(), data_to_send, 'json')
        .error(function() {
            showError('Le commentaire n’a pas pu être envoyé (erreur du serveur ou qui ne répond pas).');
        })
...
function send_data() {
    ...
    {
        showStatus('Envoi du texte… (Déplacez votre souris pour ajouter de l’entropie)…', spin=true);
    ...
    $.post(scriptLocation(), data_to_send, 'json')
        .error(function() {
            showError('Les données n’ont pas pu être envoyées (erreur du serveur ou qui ne répond pas).');
        })
        .success(function(data) {
            if (data.status == 0) {
                stateExistingPaste();
                ...
                showError('Impossible d’enregistrer le texte : '+data.message);
            }
            else {
                showError('Impossible d’enregistrer le texte.');
            }
        });
}
...
$(function() {
    ...
    if ($('div#cipherdata').text().length > 1) {
        ...
        if (window.location.hash.length == 0) {
            showError('Impossible de déchiffrer le texte : il manque la clé de chiffrement dans l’URL (Avez-vous utilisé une redirection ou un raccourcisseur d’URL qui aurait tronqué une partie de l’URL ?)');
            return;
        }
